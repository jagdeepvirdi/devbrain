...
- **Comprehensive server test coverage** — closed out every remaining zero- and partial-coverage service (`backup`, `exporter`, `notifier`, `notifications`, `audit`, `antigravity`/`claude` discovery, `session-reader`, `tasks-watcher`, `links`, `ai`, `integrations`, `parser`) and all 25 route handlers (`documents`, `issues`, `tasks`, `projects`, `auth`, `git`, `search`, `notify`, `settings`, `users`, `dashboard`, `commands`, `releases`, `runbooks`, and more). `server/vitest.config.ts`'s `coverage.thresholds` raised from the initial 37/28/34/39% baseline to the actual shipped 96/93/94/97% (statements/branches/functions/lines). See `TASKS.md` for the per-file breakdown.
- **Trend & Visibility Dashboards** — two new Dashboard widgets: **Issue Throughput**, a per-project weekly opened-vs-resolved bar chart over a 12-week window (`GET /api/dashboard/issue-throughput`), and **Embedding Health Trend**, a 30-day line chart of pending/failed embedding counts (`GET /api/dashboard/embedding-health-trend`) backed by a new hourly `embedding_health_snapshots` scheduler (`server/services/embeddingHealthSnapshot.ts`) with 30-day retention. Both render as hand-rolled SVG/CSS widgets, matching the rest of the Dashboard's no-chart-library convention.
