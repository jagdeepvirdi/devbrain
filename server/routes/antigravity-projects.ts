...
// ── POST /api/antigravity-projects/scan ────────────────────────────────────────────
...
router.post('/scan', async (_req, res) => {
  ...
  try {
    ...
    if (!scanRoot) {
      res.status(422).json({ error: 'No scan root configured. Go to Settings → Antigravity Integration.' })
      return
    }
...
// ── GET /api/antigravity-projects/:id/tasks ────────────────────────────────────────
...
// ── GET /api/antigravity-projects/:id/tasks/watch  (SSE) ───────────────────────────
...
router.get('/:id/tasks/watch', async (req, res) => {
  try {
    ...
    console.error('Antigravity task-watch error:', err)
    if (!res.headersSent) res.status(500).end()
  }
})

// ── GET /api/antigravity-projects/:id/sessions ─────────────────────────────────────
...
// ── GET /api/antigravity-projects/:id/sessions/:sessionId ──────────────────────────
