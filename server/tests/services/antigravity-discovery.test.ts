...
import { discoverProjects } from '../../services/antigravity-discovery.js'
...
describe('discoverProjects (Antigravity)', () => {
  let root: string

  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'devbrain-antigravity-discovery-'))
  })
  ...
  it('discovers a full project: marker file + TASKS.md phases + sessions + project match', async () => {
    await write(root, 'proj-full/ANTIGRAVITY.md', '')
    ...
    // it back to a real ISO string via toISOString() (see services/antigravity-discovery.ts)
  ...
  it('discovers a marker-only project with directory-name fallback and empty phases', async () => {
    await write(root, 'bare-marker/ANTIGRAVITY.md', '')
  ...
  it('keeps a quoted (non-Date) last_updated string as-is', async () => {
    await write(root, 'quoted-date/ANTIGRAVITY.md', '')
  ...
  it('skips node_modules, .gemini, and dotfolders even when they contain a marker file', async () => {
    await write(root, 'node_modules/fake-pkg/ANTIGRAVITY.md', '')
    await write(root, '.gemini/ANTIGRAVITY.md', '')
    await write(root, '.hidden/ANTIGRAVITY.md', '')
    await write(root, 'normal-proj/ANTIGRAVITY.md', '')
  ...
  it('does not recurse into an already-qualifying folder (no nested double-count)', async () => {
    await write(root, 'outer/ANTIGRAVITY.md', '')
    await write(root, 'outer/nested/ANTIGRAVITY.md', '')
  ...
  it('finds a project at depth 3 but not one nested one level deeper (maxDepth=3)', async () => {
    await write(root, 'l1/l2/proj3/ANTIGRAVITY.md', '')
    await write(root, 'l1/l2/l3/proj4/ANTIGRAVITY.md', '')
  ...
  it('falls back to the directory name gracefully when TASKS.md has malformed frontmatter', async () => {
    await write(root, 'broken/ANTIGRAVITY.md', '')
  ...
  it('matches an existing project by normalized short_name when the display name differs', async () => {
    await write(root, 'quant-cru/ANTIGRAVITY.md', '')
