...
vi.mock('../../services/antigravity-discovery.js', () => ({
...
function getHandler(routePath: string, method: 'get' | 'post') {
  const layer = (antigravityProjectsRouter as unknown as { stack: RouteLayer[] }).stack.find(
...
describe('POST /api/antigravity-projects/scan', () => {
...
describe('GET /api/antigravity-projects/:id/tasks', () => {
  beforeEach(() => vi.clearAllMocks())
...
describe('GET /api/antigravity-projects/:id/tasks/watch (SSE)', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.useRealTimers()
  })
  ...
  it('logs and responds 500 when headers have not been sent yet', async () => {
    ...
    expect(errSpy).toHaveBeenCalledWith('Antigravity task-watch error:', expect.any(Error))
    expect(res.status).toHaveBeenCalledWith(500)
    errSpy.mockRestore()
  })
...
describe('GET /api/antigravity-projects/:id/sessions', () => {
  beforeEach(() => vi.clearAllMocks())
...
describe('GET /api/antigravity-projects/:id/sessions/:sessionId', () => {
  beforeEach(() => vi.clearAllMocks())
